...
public class MountableFile implements Transferable {
    ...
    private File createTempDirectory() {
        try {
            if (isOnBitbucket()) {
                //Path tmpPath = Paths.get(getBitbucketCloneDir(), BITBUCKET_TMP_SUBDIR);
                //if (!java.nio.file.Files.exists(tmpPath)) {
                    // Files.createDirectory(tmpPath);
                //}
                Path bbClonePath = Paths.get(getBitbucketCloneDir());
                if (!java.nio.file.Files.exists(bbClonePath)) {
                    Files.createDirectory(bbClonePath);
                }
                return Files.createTempDirectory(bbClonePath, TESTCONTAINERS_TMP_DIR_PREFIX).toFile();
            }
