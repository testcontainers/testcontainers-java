...
public class GenericContainer extends FailureDetectingExternalResource implements LinkableContainer {
    ...
    public void start() {
        ...
        try {
            ...
            throw new ContainerFetchException("Failed when attempting to pull or determine image name", e);
        }
        ...
        try {
            ...
            logger().debug("Starting container: {}", imageName);

            Unreliables.retryUntilSuccess(STARTUP_RETRY_COUNT, () -> {
                attempt[0]++;
                logger().debug("Trying to start container: {} (attempt {}/{})", imageName, attempt[0], STARTUP_RETRY_COUNT);
   ...
   private void tryStart(Profiler profiler) {
       ...
       try {
           logger().debug("Starting container: {}", image.get());
    ...
    public void stop() {
        ...
        ContainerReaper.instance().stopAndRemoveContainer(containerId, this.getDockerImageName());
    }
    ...
    public void setDockerImageName(@NonNull String dockerImageName) {
        ...
        try {
            // Resolve image as soon as it's set
