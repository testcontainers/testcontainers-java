...
public class AbstractMongo {
    ...
    protected void executeTx(MongoDBContainer mongoDBContainer) {
        try (
            MongoClient mongoSyncClientBase = MongoClients.create(mongoDBContainer.getConnectionString());
            MongoClient mongoSyncClient = MongoClients.create(mongoDBContainer.getReplicaSetUrl())
        ) {
            mongoSyncClient
                .getDatabase("mydb1")
                .getCollection("foo")
            ...
            try (ClientSession clientSession = mongoSyncClient.startSession()) {
