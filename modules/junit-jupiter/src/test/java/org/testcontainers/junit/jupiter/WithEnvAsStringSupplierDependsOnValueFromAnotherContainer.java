...
public class WithEnvAsStringSupplierDependsOnValueFromAnotherContainer {
    ...
    @Container
    private static final GenericContainer<?> FIRST_CONTAINER = new GenericContainer<>(JUnitJupiterTestImages.HTTPD_IMAGE)
        .withExposedPorts(80);

    @Container
    private static final GenericContainer<?> SECOND_CONTAINER = new GenericContainer<>(JUnitJupiterTestImages.HTTPD_IMAGE)
        .withExposedPorts(80);
    ...
    public void smokeTest() {
        ...
        assertThat(getDependencyEnvOfMappedPort(FIRST_DEPENDENCY_ENV_KEY)).isEqualTo(String.valueOf(FIRST_CONTAINER.getFirstMappedPort()));
        assertThat(getDependencyEnvOfMappedPort(SECOND_DEPENDENCY_ENV_KEY)).isEqualTo(String.valueOf(SECOND_CONTAINER.getFirstMappedPort()));

    }

    private static String getDependencyEnvOfMappedPort(String dependencyEnvKey) {
        return APP_CONTAINER.getEnvMap().entrySet().stream().filter(e -> e.getKey().startsWith(dependencyEnvKey)).map(e -> e.getValue().get()).findFirst().orElse(null);
    }
}
